...
def pixel_down_shuffle(x, downsacale_factor):
    ...
    input_view = x.contiguous().view(batchsize, num_channels, out_height,
                                     downsacale_factor, out_width,
                                     downsacale_factor)
...
def iwt_init(x):
    ...
    h = torch.zeros(
        [out_batch, out_channel, out_height, out_width]).float().to(device=x.device)
...
class DBlock_com(nn.Module):
    def __init__(
        ...
        m.append(act)
        m.append(
            conv(in_channels, out_channels, kernel_size, bias=bias, dilation=3))
        if bn: m.append(nn.BatchNorm2d(out_channels, eps=1e-4, momentum=0.95))
        m.append(act)
...
class DBlock_com1(nn.Module):
    def __init__(
        ...
        m.append(act)
        m.append(
            conv(in_channels, out_channels, kernel_size, bias=bias, dilation=1))
        if bn: m.append(nn.BatchNorm2d(out_channels, eps=1e-4, momentum=0.95))
        m.append(act)
